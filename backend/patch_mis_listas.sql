-- ============================================================================
-- Parche: "Mis listas" (playlists M3U completas, editables desde el panel admin)
-- Ejecuta TODO esto en Supabase → SQL Editor → Run
-- ============================================================================

-- sv_config ahora también devuelve las playlists
...
returns json language sql security definer set search_path = public, extensions as $$
  select json_build_object(
    ...
    'playlists',      (select value from app_config where key = 'custom_playlists')
  );
$$;

-- Guardar las playlists (protegido por el token de administrador)
create or replace function sv_set_playlists(p_token text, p_lists text)
...
begin
  ...
  insert into app_config(key, value) values ('custom_playlists', p_lists)
    on conflict (key) do update set value = excluded.value;
  return json_build_object('ok', true);
end $$;

grant execute on function sv_config()                 to anon, authenticated;
grant execute on function sv_set_playlists(text,text) to anon, authenticated;

insert into app_config(key,value) values ('custom_playlists', '[]') on conflict (key) do nothing;
